...
(define (reduce-reverse proc init list)
  (if (null? list) init
      (reduce-reverse proc (proc (car list) init) (cdr list))))
; ; Testing
(display "b)")(newline)
...
(define (nth2 index items)
  (cond ((>= index (length items)) (display "Index out of range!")(newline))
        ((= index 0) (car items))
        (else (nth2 (- index 1) (cdr items)))))

; ; If I don't want to check if the index is out of bounds, I can write it like this:
(define (nth3 index items)
  (if (= index 0) (car items)
      (nth3 (- index 1) (cdr items))))

; ; Testing both nth and nth2
(display "d)")(newline)
(nth 4 '(1 2 3 4 5 6 7 8))
(nth2 4 '(1 2 3 4 5 6 7 8))
(nth3 4 '(1 2 3 4 5 6 7 8))
...
(define (where num list)
  (define (where-iter list cnt)
    (cond ((null? list) #f)
          ((= (car list) num) cnt)
          (else (where-iter (cdr list) (+ 1 cnt)))))
  (where-iter list 0))
