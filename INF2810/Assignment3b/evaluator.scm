...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((lett? exp) (mc-eval (lett->lambda exp) env))))
...
; Gets the body of the let expression
(define (let-body exp)
  (cddr exp))

; Gets the definitions of the let expression
(define (let-definitions exp)
  (cadr exp))

; Gets the first variable name from the definitions
(define (let-define-var exp)
  (caar exp))

; Gets the first expression from the definitions
(define (let-define-expr exp)
  (cadar exp))

(define (let->lambda exp)
  (define (get-params exp)
    (if (null? exp)
      '()
      (cons (let-define-var exp) (get-params (cdr exp)))))

  (define (get-values exp)
    (if (null? exp)
      '()
      (cons (let-define-expr exp) (get-values (cdr exp)))))

  (cons (make-lambda
          (get-params (let-definitions exp))
          (let-body exp))
        (get-values (let-definitions exp))))
...
(define (lett-expr exp)
  (caddr exp))

(define (lett-definitions exp)
  (cdr exp))

(define (lett-continue? exp)
  (eq? 'and (cadddr exp)))

(define (lett-next-body? exp)
  (eq? 'in (cadddr exp)))

(define (lett-next exp)
  (cddddr exp))

(define (lett->lambda exp)
  (define (get-params exp)
    (if (lett-continue? exp)
      (cons (lett-var exp) (get-params (lett-next exp)))
      (cons (lett-var exp) '())))

  (define (get-values exp)
    (if (lett-continue? exp)
      (cons (lett-expr exp) (get-values (lett-next exp)))
      (cons (lett-expr exp) '())))

  (define (get-body exp)
    (if (lett-next-body? exp)
      (lett-next exp)
      (get-body (lett-next exp))))

  (cons (make-lambda
          (get-params (lett-definitions exp))
          (get-body (lett-definitions exp)))
        (get-values (lett-definitions exp))))
...
(define (eval-if exp env)
  (if (true? (mc-eval (if-predicate exp) env))
    ...
    (if (elsif? (if-next-expr exp))
      (eval-if (if-next-expr exp) env)
...
(define (if-next-expr exp) (cddddr exp))

(define (if-alternative exp)
  (if (not (null? (cdr (cddddr exp))))
      (cadr (cddddr exp))
      'false))
...
(define (lett? exp) (tagged-list? exp 'lett))
